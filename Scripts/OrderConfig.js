					...
					$.each(data,function(item,value)
						{
							...
							$(".ListTable").append("<tr onClick=Modify('"+value.OrderId+"') ><td >"+value.OrderCode+"</td><td>"+value.CustomerName+"</td><td >"+value.SellerName+"</td><td style='width:20%;display:none;'>"+value.OrderStatusName+"</td><td >"+(value.OrderDate ==null?value.CreateDate:value.OrderDate).substring(0,10)+"</td></tr>");
    ...
    xhr.onreadystatechange = function(){
     ...
     /*    $li.find(".progress span").animate({'width': "100%"}, pecent < 95 ? 200 : 0, function(){
      ...
      if(xhr.status != 200)
      {
      	...
      	$li.remove();
      	$.growl.notice({ title: "异常提示", message: "图片上传失败，检查网络!CODE:"+xhr.status,location:"middle", duration:1500});
      }
    };
    //数据发送进度，前50%展示该进度
