...
$(document).ready(function(e) {
	...
	fistload = true;
	var FinishDateHtml = "<li class='aui-list-item' id='ModifyFinishDate'><div class='aui-list-item-inner'><div class='aui-list-item-label'>维修日期修改</div><div class='aui-list-item-input'><input type='date' id='FinishDate' placeholder='维修日期'></div></div></li>"
	...
	//GetPositionCodeCode();
	$("input[name=radio][data-text='未维修']").prop('checked',true);
	GetOrderList();
...
//      图片加载完毕之后进行压缩，然后上传
       ...
       function callback(){
		  ...
		  /*  var imgfiles = []; 
            ...
            //上传应用图标
            $.ajaxFileUpload({ 
                url: configurl+"/api/$.ajax/UploadOrderPhoto",
...
function GetOrderList(){
	...
	if(fistload==true)
	{
		...
		fistload = false;
     	...
     	var json = {loginid: UserId, ParamCommon:conditioncommon,StartTime:$("#StartDate").val(),EndTime:$("#EndDate").val(),SellerName:SellerNameFilter,CompanyName:CompanyNameFilter,OrderStatusName:statusName};
     ...
     $.ajax({
         		type:"get",
         		url:configurl+"/api/Repairment/GetListJson",
              ...
              success: function (data)
              {
                ...
                $(".ListTable tr").remove()
					...
					$(".ListTable").append("<tr ><td style='width:35%;'>维修编码</td><td>客户</td><td >用户</td><td style='width:20%;display:none;'>订单状态</td><td>订单日期</td></tr>");
   ...
   function Add(){
	 ...
	 $('#Add :input').val("");
	 $("input[name=OrderStatusName][data-text='未维修']").prop('checked',true);
	  $("input[type=radio][name=OrderStatusName][data-text=未维修]").prop('disabled',false);
	 $(".img-list li").not(':eq(0)').remove();
	   $("input[type=radio][name=OrderStatusName][data-text=已维修]").css('background-color','')
				h=1;
...
function Modify(paraOrderId){
     ...
     $(".aui-title").text("编辑订单");
	  ...
	  {
			type:"get",
			url:configurl+"/api/Repairment/GetRepairmentFormJson",
...
//          获取图片大小
				...
				$.each(data.orderEntry,function(name,item)
				{
					 ...
					 var rowTem = '<tr class="tr_' + index + '">'
					   ...
					   + '<td ><select name="StyleCodeId"  onchange="SetPrice(this)"><option>请选择</option></select><span ><input type="text" name="OpPositionId"  placeholder="施工位置" /></span></td>'
					...
					rowObj.find('input[name=OpPositionId]').val(item.OpPosition)
				...
				if(data.order.OrderStatusName=='已维修')
				{
					//
					IsFinish = 1;
					...
					$("input[type=radio][name=OrderStatusName][data-text=已维修]").css('background-color','#03a9f4')
					
					 
					$("input[type=radio][name=OrderStatusName][data-text=未维修]").prop('readonly','readonly');
					$("input[type=radio][name=OrderStatusName][data-text=未维修]").prop('disabled',true);
       ...
       // var orderEntryJsonO = [{ "ProductName": 'ProductName1' }, { "ProductName": 'ProductName1'}];//object类型
        var orderEntryJson =  JSON.stringify(GetOrderEntryList());//string类型
			var orderClassJson={ userid: UserId, username: UserName, keyValue: OrderId, orderJson:orderJson, orderEntryJson:orderEntryJson,receivableEntryJson:receivableEntryJson}
                $.ajax({
                    url:  configurl+"/api/Repairment/SaveOrderFormP",
                    data: orderClassJson,
										async:true,
                    type: "post",
                    dataType: "json",
...
function upload(basestr, type, $li,filename){
	...
	xhr.open('post',configurl+'/api/Repairment/UploadOrderPhoto?userid='+UserId+'&username='+UserName+'&orderid='+OrderId)
...
function GetOrderEntryList()
		...
		console.log(productIndex)
		  ...
		  data["BrandId"] =SelectProduct.Brand;
		   ...
		   data["ColorCodeId"] =$(this).find('select[name=ColorCodeId]').val();
			...
			data["OpPositionId"] =$(this).find('input[name=OpPositionId]').val();
		    data["OpPosition"] =$(this).find('input[name=OpPositionId]').val();
  ...
  function GetProduct()
	...
	function GetStyleCode(ProductCode)
	{
			...
			$.ajax({ 
                    url: configurl+"/api/Repairment/GetStyleJson",                      
                    type: "GET",
					async:false,					
	...
	function GetColorCode(ProductCode)
	{
			...
			$.ajax({ 
                    url: configurl+"/api/Repairment/GetColorJson",                      
                    type: "GET",
					async:false,					
                    data:{ProductCode :ProductCode},   
                    dataType: "json",
